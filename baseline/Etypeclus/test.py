...
def get_top_n_instance(net, args, dataloader, n=5):
    ...
    with torch.no_grad():
        ...
        centers = torch.zeros(args.new_class, 100, device=device)
        ...
        for iteration, data in enumerate(dataloader):
            ...
            true_label = data[3]
            data = data[:-2]
            sia_rep, logits = net.module.encoder(data)
...
def main(args):
    ...
    net = torch.load(args.save+"model_Etypeclus_"+ args.dataset +"_"+ str(args.seed) +".pt").cuda()
    epoch=0
    print("net ready...")
    print("-"*32)
