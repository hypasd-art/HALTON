...
class InputFeatures(object):
  ...
  def __init__(self, unique_id, tokens, input_ids, input_mask,pos_span,valid_mask,mask_span): # , input_type_ids
    self.unique_id = unique_id
    self.tokens = tokens
...
class Labeled_Dataset(Dataset):
    def __init__(self,args, examples, tokenizer):
        ...
        print(len(self.examples))
        self.features = self.convert_examples_to_features(examples=self.examples, seq_length=min(2+actual_max_len, self.max_len)+20, tokenizer=self.tokenizer)
...
class unLabeled_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = self.examples[index].label
        pos_span = self.features[index].pos_span
        pseudo1 = self.examples[index].pseudo1
        pseudo2 = self.examples[index].pseudo2
        mask_span = self.features[index].mask_span
        return input_ids, input_mask, valid_mask, label, pos_span, mask_span, index, pseudo1, pseudo2
    ...
    def collate_fn(data):
        data = list(zip(*data))
        input_ids, input_mask, valid_mask, label, pos_span,mask_span, index, pseudo1, pseudo2 = data
        input_ids = torch.stack(input_ids, dim = 0) # (batch_size, L)
        input_mask = torch.stack(input_mask, dim = 0)
        valid_mask = torch.stack(valid_mask, dim = 0)
        label = torch.LongTensor(label) # (batch_size)
        ...
        mask_span = torch.LongTensor(mask_span) # (batch_size, 2)
        index = torch.LongTensor(index)
        pseudo1 = torch.LongTensor(pseudo1)
        pseudo2 = torch.LongTensor(pseudo2)
        return input_ids, input_mask, valid_mask, label, pos_span, mask_span, index, pseudo1, pseudo2
