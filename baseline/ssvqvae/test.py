...
# 选取簇中心的那个样本进行命名，利用PLM的filling the blank的能力
...
def get_top_n_instance_gold(net, args, dataloader, n=5):
    ...
    with torch.no_grad():
        ...
        for iteration, data in enumerate(dataloader):
            ...
            logits = net.module.encoder(data)
            sia_rep= net.module.get_feat(data)
            label_pred = logits[:,args.num_class:].max(dim = -1)[1].cpu()
...
def main(args):
    ...
    net = torch.load(args.save+"model_ssvqvae_"+ args.dataset +"_"+ str(args.seed) +".pt").cuda()
    epoch=0
    print("net ready...")
    print("-"*32)
